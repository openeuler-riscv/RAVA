...
## 为 LAVA worker 添加 Proxy

参考 [Using the HTTP proxy](https://lava.oerv.ac.cn/static/docs/v2/proxy.html#using-the-http-proxy)，LAVA dispatcher 会读取环境变量中的 `HTTP_PROXY`。这些环境变量需要在 **server 侧**的 `/etc/lava-server/env.yaml` 或 `/etc/lava-server/dispatcher.d/<worker-name>/env.yaml` 中配置，然后由 server 下发给对应的 dispatcher/worker 使用。

如果所有 worker 都使用同一个 proxy，直接在 server 上编辑全局配置：

```Bash
sudo cp /etc/lava-server/env.yaml /etc/lava-server/env.yaml.$(date +%F-%H%M%S).bak
sudo vim /etc/lava-server/env.yaml
```

确认 `overrides` 下包含 proxy 相关变量，示例如下：
...
overrides:
  ...
  no_proxy: localhost,127.0.0.1,::1,<lava-server-ip>,<lava-server-domain>,<worker-ip>,<inner-repo>
  PATH: /usr/local/bin:/usr/local/sbin:/bin:/usr/bin:/usr/sbin:/sbin
```

其中：

* `<proxy-host>:<proxy-port>` 替换为 worker 可以访问到的 proxy 地址，如 `10.20.193.10:3128`
* `NO_PROXY` / `no_proxy` 建议加入 LAVA server 地址、域名以及 worker 本机地址，内部仓库等，多个地址用逗号分隔，避免 worker 访问 server、本机 Apache/NFS/TFTP 等内部地址时绕到 proxy
* 如果 proxy 需要认证，可写成 `http://<user>:<password>@<proxy-host>:<proxy-port>`，但注意该文件会成为 server 上的敏感配置文件

如果不同 worker 需要使用不同 proxy，或者只想对指定 worker 生效，则为每个 worker 在 server 上创建独立配置。`<worker-name>` 必须与 worker 注册到 LAVA 时使用的名称一致，也就是 `/etc/lava-dispatcher/lava-worker` 中 `WORKER_NAME` 的值，或 Web 页面 `Scheduler > Workers` 中显示的名称。

对每个 worker 分别创建配置文件：

```Bash
WORKERS="cipu-zz-debian-01 lava-worker-02 lava-worker-03 lava-worker-lpi4a"

for worker in $WORKERS; do
    sudo install -d -m 0755 "/etc/lava-server/dispatcher.d/${worker}"
    sudo tee "/etc/lava-server/dispatcher.d/${worker}/env.yaml" >/dev/null <<'EOF'
purge: true
...
> 使用 `/etc/lava-server/dispatcher.d/<worker-name>/env.yaml` 时，LAVA 会优先使用该 worker 专属的环境配置；没有专属配置时，才回退到 `/etc/lava-server/env.yaml`。因此专属配置中不要只写 proxy 变量，也要保留 `LC_ALL`、`LANG`、`PATH` 等基础变量。

修改完成后，建议在没有 job 运行的维护窗口重启 server 侧服务，确保后续新 job 使用新的环境配置：
...
如果某个 worker 正在执行 job，先等待 job 完成；新配置只对后续启动的 job 生效。随后提交一个会下载外部资源的简单 job 验证，重点检查 job log 中下载步骤是否正常，以及 worker 上的日志：

```Bash
sudo tail -f /var/log/lava-dispatcher/lava-worker.log
```

需要注意的是：

* proxy 地址必须从 worker 网络可达，而不是只从 server 可达
* 如果下载的是 `https://` 资源，通常还需要同时配置 `HTTPS_PROXY` / `https_proxy`
* 如果 job 需要访问内网仓库、LAVA server 或 worker 本机资源，把这些地址加入 `NO_PROXY` / `no_proxy`
